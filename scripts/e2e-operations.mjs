async (page) => {
  ...
  async function waitForQuotesReady(min = 0) {
    ...
    await page.waitForFunction((n) => document.querySelectorAll("article").length >= n, min, {
      timeout: 20000,
    });
  }
  ...
  await page.waitForFunction((n) => document.querySelectorAll("article").length === n, before + 1);
  ...
  // ── Authors page ──────────────────────────────────────────────────────
  await navTo("authors");
  await page.waitForFunction(() => document.querySelectorAll(".author-card").length > 0, {
    timeout: 15000,
  });
  ...
  pass(
    "匯入 JSON 回饋",
    importFeedback.includes("匯入完成") || importFeedback.includes("新增"),
    importFeedback,
  );
  ...
  const importVisible =
    (await page.locator("article").filter({ hasText: importQuoteText }).count()) > 0;
  pass(
    "匯入 JSON 顯示名言",
    importVisible && countAfterImport === countBeforeImport + 1,
    `${countBeforeImport} → ${countAfterImport}`,
  );

  // Pets toggle (causes full reload)
  await openSettings();
  const petsToggle = page
    .locator(".setting-row")
    .filter({ hasText: "網頁小夥伴" })
  ...
  pass(
    "設定頁主題切換",
    themeBeforeSettings !== themeAfterSettings,
    `${themeBeforeSettings} → ${themeAfterSettings}`,
  );
  ...
  pass(
    "頂欄主題切換",
    themeBeforeHeader !== themeAfterHeader,
    `${themeBeforeHeader} → ${themeAfterHeader}`,
  );
  ...
  const navEn = await page
    .locator("nav a")
    .evaluateAll((els) => els.map((a) => a.textContent.trim()));
  pass(
    "英文導覽：Quotes",
    navEn.some((t) => t.includes("Quotes")),
    JSON.stringify(navEn),
  );
