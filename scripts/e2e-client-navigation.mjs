async (page) => {
  ...
  await page.waitForFunction(() => (window.__wq?.swapCount ?? 0) >= 1, undefined, {
    timeout: 5000,
  });
  ...
  const afterAuthors = await page.evaluate(() => ({
    ...
    activeNav:
      document.querySelector(".site-nav a.nav-link.active")?.getAttribute("data-nav-id") ?? null,
  }));

  await page.locator('.site-nav a[data-nav-id="settings"]').click();
  await page.waitForURL(/\/settings\/?/);
  await page.waitForFunction(() => (window.__wq?.swapCount ?? 0) >= 2, undefined, {
    timeout: 5000,
  });
  ...
  const checks = {
    ...
    headerPersisted: afterAuthors.sameHeaderNode && afterSettings.sameHeaderNode,
